    ...
    <div class="py-8 pt-0 mt-2">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg sm:rounded-lg p-6">
                ...
                <form action="{{ route('offers.update', ['id' => $offer->id]) }}" method="POST" class="space-y-4" id="editOfferForm">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700">Tytuł*</label>
                        <input type="text" name="title" id="title" value="{{ $offer->name }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-pink-500 focus:border-pink-500" required>
                    </div>
                    <div>
                        <label for="description" class="block text-sm font-medium text-gray-700">Opis</label>
                        <textarea name="description" id="description" rows="4" placeholder="np. założone kilka razy, rozmiar zgodny z metką" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-pink-500 focus:border-pink-500">{{$offer->description}}</textarea>
                    </div>

                    <div>
                        <label for="condition" class="block text-sm font-medium text-gray-700">Stan*</label>
                        <textarea name="condition" id="condition" rows="4" class="mt-1 block w-full h-11 border-gray-300 rounded-md shadow-sm focus:ring-pink-500 focus:border-pink-500" required>{{$offer->condition}}</textarea>
                    </div>

                    <div>
                        <label for="price" class="block text-sm font-medium text-gray-700">Cena*</label>
                        <input type="number" name="price" id="price" value="{{ $offer->price }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-pink-500 focus:border-pink-500" min="0" required>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="size" id="size" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-pink-500 focus:border-pink-500" required>
                            ...
                            @foreach($sizes as $size)
                                <option value="{{ $size->id }}" {{ $offer->size_id == $size->id ? 'selected' : '' }}>
                                    {{ $size->name }}
                                </option>
                            @endforeach
                        </select>
